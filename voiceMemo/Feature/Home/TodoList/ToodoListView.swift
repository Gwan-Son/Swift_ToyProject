...
// MARK: - Todo 셀 뷰
private struct TodoCellListView: View {
    @EnvironmentObject private var todoListViewModel: TodoListViewModel
    @State private var isRemoveSelceted: Bool
    ...
    fileprivate init(
        ...
        _isRemoveSelceted = State(initialValue: isRemoveSelected)
        self.todo = todo
    }
        ...
        VStack(spacing: 20) {
            HStack {
                ...
                if todoListViewModel.isEditTodoMode {
                    Button(
                        action: {
                            isRemoveSelceted.toggle()
                            todoListViewModel.todoRemoveSelectedBoxTapped(todo)
                        },
                        label: {
                            isRemoveSelceted ? Image("selectedBox") : Image("unSelectedBox")
                        }
                    )
                }
            }
